...
class EmbeddingService(BaseService):
    ...
    def load_dense_model(self) -> SentenceTransformer:
        ...
        """
        logger.info(f"Loading SentenceTransformer model from {self.setting.embedding.dense_model_path}")
        return SentenceTransformer(self.setting.embedding.dense_model_path)
    ...
    def load_sparse_model(self) -> SparseTextEmbedding:
        ...
        """
        logger.info(f"Loading SparseTextEmbedding model from {self.setting.embedding.sparse_model_path}")
        return SparseTextEmbedding(self.setting.embedding.sparse_model_path)
    ...
    def _get_embeddings_batch(self, texts: List[str]) -> List[List[float]]:
        ...
        embeddings = []
        for i in range(0, len(valid_texts), self.setting.embedding.max_token_limit):
            batch = valid_texts[i:i + self.setting.embedding.max_token_limit]
            try:
                batch_embeddings = self.load_dense_model.encode(batch).tolist()
                embeddings.extend(batch_embeddings)
            except Exception as e:
                logger.error(f"Error in batch {i//self.setting.embedding.max_token_limit}: {str(e)}")
