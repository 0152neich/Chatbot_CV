...
if st.sidebar.button('OK'):
    if name_input.strip():
        ...
        st.sidebar.success(
            f'Tên ứng viên đã xác nhận: {st.session_state.candidate_name}',
        )
    else:
...
if st.session_state.name_confirmed and st.session_state.candidate_name:
    st.sidebar.info(
        f'Ứng viên hiện tại: **{st.session_state.candidate_name}**',
    )
